...
_VERDICT_HEX = {"bullish": "#3fb950", "bearish": "#f85149", "neutral": "#8b949e"}


def build_signals_sidebar(signals: list, verdict: dict) -> str:
    """Sidebar HTML: a BUY/SELL verdict badge plus the per-signal posture list."""
    vcolor = _VERDICT_HEX.get(verdict["state"], "#8b949e")
    rows = ""
    for s in signals:
        c = _VERDICT_HEX[s["state"]]
        icon = "▲" if s["state"] == "bullish" else ("▼" if s["state"] == "bearish" else "—")
        rows += (
            f'<div class="sig-row">'
            f'<span class="sig-dot" style="color:{c};">{icon}</span>'
            f'<div class="sig-text"><div class="sig-name">{html_lib.escape(s["name"])}</div>'
            f'<div class="sig-detail">{html_lib.escape(s["detail"])}</div></div></div>'
        )
    if not rows:
        rows = '<div class="sig-detail">Not enough data for signals.</div>'
    return (
        f'<div class="side-title">📊 Signal Summary</div>'
        f'<div class="verdict-badge" style="background:{vcolor};">{verdict["label"]}'
        ...
        f'<div class="sig-list">{rows}</div>'
        f'<div class="side-note">Mechanical signals (trend, RSI, MACD, Bollinger, '
        f'momentum) — verify before acting.</div>'
    )
...
def build_html(df: pd.DataFrame, info: dict, period: str,
  ...
  <div style="text-align:center;padding:60px 20px;color:var(--text2);">
    ...
    # ── Signals sidebar (current posture → BUY/SELL verdict) ─────
    _signals = evaluate_signals(df)
    sidebar_html = build_signals_sidebar(_signals, signal_verdict(_signals))
...
<head>
  ...
  <style>
    ...
    *{{box-sizing:border-box;margin:0;padding:0;}}
    ...
    .sidebar{{width:280px;flex-shrink:0;border-left:1px solid var(--border);
      padding:20px 18px;position:sticky;top:0;align-self:flex-start;
    ...
    .side-title{{font-size:12px;font-weight:600;color:var(--text2);text-transform:uppercase;
      letter-spacing:.5px;margin-bottom:12px;}}
    .verdict-badge{{display:flex;justify-content:space-between;align-items:center;color:#0d1117;
      font-weight:800;font-size:16px;letter-spacing:.5px;padding:12px 14px;border-radius:8px;}}
    .verdict-badge .vscore{{font-size:11px;font-weight:700;opacity:.85;}}
    .vcount{{font-size:11px;color:var(--text2);margin:8px 0 14px;}}
    .sig-list{{display:flex;flex-direction:column;gap:8px;}}
    .sig-row{{display:flex;gap:10px;align-items:flex-start;background:var(--bg3);
      border:1px solid var(--border);border-radius:6px;padding:8px 10px;}}
    .sig-dot{{font-size:13px;font-weight:700;line-height:1.4;}}
    .sig-name{{font-size:12px;font-weight:600;}}
    .sig-detail{{font-size:11px;color:var(--text2);}}
    .side-note{{font-size:10px;color:var(--text2);margin-top:14px;font-style:italic;}}
